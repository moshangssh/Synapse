...
## 2025-08-04: 决定实现SRT解析工具功能

**决策背景:**

用户需要为字幕编辑器应用添加SRT文件解析功能，以便能够导入和处理SRT格式的字幕文件。应用需要一个可靠的工具来解析SRT文件内容，验证其格式，并提供错误处理机制。

**决策过程:**

1.  **需求分析 (code-developer):**
    *   分析了SRT文件格式标准，确定了需要支持的功能：解析标准SRT格式、处理BOM标记、验证时间码格式、错误处理等。
    *   确定了需要实现两个主要函数：`parseSRTFile`用于解析SRT文件内容，`validateSRTContent`用于验证SRT文件内容的有效性。

2.  **技术方案设计 (code-developer):**
    *   设计了基于正则表达式的SRT解析方案，能够处理各种SRT格式变体。
    *   实现了详细的错误处理和验证机制，提供清晰的错误信息。
    *   创建了全面的测试用例，确保功能的可靠性和稳定性。

**最终决策:**

**批准实现SRT解析工具功能，包括`parseSRTFile`和`validateSRTContent`函数，以及相应的测试用例。**

**决策理由:**

*   **满足核心需求:** SRT文件解析是字幕编辑器的核心功能，实现此功能能够显著提升应用的实用性。
*   **技术可靠性:** 基于正则表达式的解析方案经过充分测试，能够处理各种SRT格式变体。
*   **用户体验:** 提供详细的错误信息和验证机制，帮助用户快速识别和修复SRT文件中的问题。
*   **代码质量:** 实现了全面的测试用例，确保功能的可靠性和稳定性。

**后续行动:**

由 `code-developer` 角色执行具体的代码实现，包括：
*   创建`frontend/synapse/src/utils/srtParser.ts`文件，实现SRT解析功能。
*   创建`frontend/synapse/src/utils/srtParser.test.ts`文件，实现全面的测试用例。
*   更新内存银行文件记录实现细节。

---

### 代码实现 [SRT文件导入功能]
[2025-08-04 11:45:00] - 在FileExplorer组件中添加SRT文件导入功能

**实现细节：**
1. 在FileExplorer组件的刷新按钮旁边添加了导入按钮，使用Material-UI的Upload图标
2. 实现了文件选择和导入逻辑：
   - 创建隐藏的文件输入元素，用于选择SRT文件
   - 添加导入按钮的点击事件处理
   - 读取选中的SRT文件内容
   - 使用parseSRTFile函数解析文件内容
   - 将解析后的数据添加到状态管理中
3. 添加了错误处理和用户反馈：
   - 文件类型验证（只接受.srt文件）
   - SRT内容格式验证
   - 使用Snackbar显示导入结果和错误信息
4. 在FileExplorer组件中显示导入的SRT文件：
   - 导入的文件显示在时间线轨道上方，使用不同的颜色区分
   - 点击导入的文件可以加载其字幕内容到主编辑区
   - 实现了文件选择状态管理，确保导入文件和时间线轨道的选择状态不会冲突
...
## 2025-08-04T12:22:24.221Z - 代码重构决策
**决策内容**: 提取重复的 SRT 字幕转换逻辑到独立工具函数
**决策原因**:
- FileExplorer.tsx 中存在两段完全相同的 SRT 字幕转换代码
- 代码重复导致维护困难和不一致性风险
- 需要统一转换逻辑，便于未来维护和修改

**实施方案**:
- 在 `frontend/synapse/src/utils/converter.ts` 中创建 `convertSrtToSubtitles` 工具函数
- 替换 FileExplorer.tsx 中的两处重复代码
- 保持原有功能不变，仅重构代码结构

**影响范围**:
- 新增文件: `frontend/synapse/src/utils/converter.ts`
- 修改文件: `frontend/synapse/src/components/layout/FileExplorer.tsx`
- 无功能变更，纯代码重构
