...
export function ResultReviewModal({
  ...
  const transformDiffs = useCallback((backendDiffs: Array<{ type: 'added' | 'removed' | 'normal'; value: string }>): DiffPart[] => {
  ...
  const handleApplyAll = useCallback(async () => {
    ...
    try {
      ...
      const optimizedSubtitles: Subtitle[] = optimizationResult.data.map(item => {
        const originalSubtitle = currentSubtitles.find(sub => sub.id === item.id);
        return {
          id: item.id,
          startTimecode: originalSubtitle?.startTimecode || '',
          endTimecode: originalSubtitle?.endTimecode || '',
